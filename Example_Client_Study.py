...
class RemoteNaiveHunterStrategy:
    ...
    def processObservation(self, fulljson, sendData):
        ...
        #PÁLYA KIVÁLASZTÁSA
        newpath=self.path[self.games%5]
        print(newpath)

        #ÚJ JÁTÉK
        if(self.games_before!=self.games):
            self.tenscores=self.tenscores+self.score
            self.games_before=self.games
            # print("score")
            # print(self.score)
            # empty=1000+empty
            # self.games_before=self.games
            # open1= open(r"E:\adaptivegame\src\ThirdOutput.txt","a")
            # open1.write("games: "+str(self.games)+" games_before: "+str(self.games_before)+"---\n")

            #JÖNNEK AZ ÚJABB PARAMÉTEREK
            if self.games>0 and self.games%10==0:
                self.scores.append(self.tenscores)
                # open1= open(r"E:\adaptivegame\src\ThirdOutput.txt","a")
                # open1.write("games: "+str(self.games)+" tenscores: "+str(self.tenscores)+"\n")
                self.tenscores=0

            #ÚJ PARAMÉTERHALMAZ LÉTREHOZÁSA KÖVETKEZIK
            if self.games>0 and self.games%80==0:
                open1= open(r"E:\adaptivegame\src\ThirdStudy.txt","a")
                #A HASZNÁLT PARAMÉTEREK ÉS AZ AZOKKAL KAPOTT EREDMÉNYEK KIÍRATÁSA
                open1.write("------------------------------------\n")
                l=0
                for i in self.params:
                    #print(i[l])
                    open1.write(str(i[0])+";"+str(i[1])+";"+str(i[2])+";"+str(i[3])+";"+str(i[4])+";"+str(i[5])+";"+str(i[6])+";"+str(i[7])+";"+str(i[8])+";"+str(i[9])+";"+str(i[10])+";"+str(i[11])+";"+str(i[12])+";"+str(i[13])+";"+str(self.scores[l])+"\n")
                    l=l+1
                open1.write(str(l)+"\n")
                open1.write(str(self.games)+"\n")

                get_bin = lambda x, n: format(x, 'b').zfill(n)


                #SZELEKCIÓ, A 4 LEGJOBB KIVÁLASZTÁSA
                list=self.scores
                params=self.params
                maxes=[]
                goodparams=[]
                ...
                for i in range(4):
                    ...
                    goodparams.append(params[idx])
                    params=params[0:idx]+params[idx+1:]


                #KERESZTEZÉS
                ran1=random.randrange(0,3)
                ran2=random.randrange(0,3)
                # print("EZ: "+str(ran1)+str(ran2))
                while ran2 == ran1:
                    ran2=random.randrange(0,3)
                #   print(ran2)
                # print("EMEZ: "+str(ran2))
                ...
                for i in goodparams[ran1]:
                    binary=get_bin(i, 15)
                    binarray1.append(binary)

                for i in goodparams[ran2]:
                    binary=get_bin(i, 15)
                    binarray2.append(binary)

                # print(binarray1)
                # print(binarray2)

                oldgoodparams=goodparams

                if ran1<ran2:
                    goodparams=goodparams[:ran1]+goodparams[ran1+1:ran2]+goodparams[ran2+1:]
                else:
                    goodparams=goodparams[:ran2]+goodparams[ran2+1:ran1]+goodparams[ran1+1:]
                    # print(goodparams)

                for i in goodparams[0]:
                    binary=get_bin(i, 15)
                    binarray3.append(binary)

                for i in goodparams[1]:
                    binary=get_bin(i, 15)
                    binarray4.append(binary)
                ...
                for i in range(0,14):
                    #   print("--")
                    cross=random.randrange(1,13)
                    #   print(cross)
                    #   print(binarray1[i])
                    newbinarray1.append(binarray1[i][:cross]+binarray2[i][cross:])
                    #   print(newbinarray1[i])
                    #   barray1.append(binarray1[i])
                    #   print(binarray2[i])
                    newbinarray2.append(binarray2[i][:cross]+binarray1[i][cross:])
                    #   print(newbinarray2[i])
                    cross=random.randrange(1,13)
                    newbinarray3.append(binarray3[i][:cross]+binarray4[i][cross:])
                    newbinarray4.append(binarray4[i][:cross]+binarray3[i][cross:])

                #MUTÁCIÓ
                for i in newbinarray1:
                    mutate=random.randrange(1,1000)
                    if mutate<101:
                        print("mutate1")
                        mu=random.randrange(0,14)
                        print(type(i[mu]))
                        if i[mu]=="1":
                            i=i[:mu]+'0'+i[mu+1:]
                        else:
                ...
                for i in newbinarray1:
                    # print(int(i,2))
                    i=int(i,2)
                    newdecimal1.append(i)
                for i in newbinarray2:
                    # print(int(i,2))
                    i=int(i,2)
                    newdecimal2.append(i)

                for i in newbinarray3:
                    # print(int(i,2))
                    i=int(i,2)
                    newdecimal3.append(i)

                for i in newbinarray4:
                    i=int(i,2)
                    newdecimal4.append(i)

                oldgoodparams.append(newdecimal1)
                oldgoodparams.append(newdecimal2)
                oldgoodparams.append(newdecimal3)
                oldgoodparams.append(newdecimal4)
                self.params=oldgoodparams
                #print(self.params)
                self.scores=[]
        ...
        if fulljson["type"] == "leaderBoard":
            ...
            sendData(json.dumps({"command": "GameControl", "name": "master",
                                 "payload": {"type": "reset", "data": {"mapPath": newpath, "updateMapPath": newpath}}}))
